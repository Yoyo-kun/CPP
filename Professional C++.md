...
> - 预处理指令以    **`#`**    字符开始；
>
> - 在预处理的时候，会把 `.h` 文件所对应的内容全部复制到对应的 `#include "headfile.h"` 复制到向相应的位置上；【始终铭记这一点，虽然这部分知识可以被编程习惯所规避掉，不过这有助理解编译和链接的过程，并且可以更清晰地理解为何要使用那样的编程习惯（比如，如何避免歧义链接错误、inline 函数不在 `.h` 文件中被再次标注）】
>
...
> ```
>
> 在这个例子中，`std::cerr` 输出错误信息 `"Not right!"`。虽然它输出到同一终端，但它可以与 `std::cout` 分开处理（例如，重定向到不同的文件）。
...
>
>   **可以使用它执行一些在技术上不被 C++ 规则允许，但是在某些情况下程序员又需要的类型转换。**
>
>   例如，可将某种引用类型转换为其他引用类型，即使这两个引用并不相关，同样地，可将某种指针类型转换为其他指针类型，即使这两个指针并不存在继承层次上的关系。
>
>   这种用法经常用于将指针转换为 void* ；这可隐式完成，不需要进行显式转换。但将  void* 指针转换为正确类型的指针需要 reinterprete_cast()。因为 void* 指针指向内存的某个位置，viod* 指针没有相关的类型信息。下面是示例：
>
>   ```C++
>   class X();
>   class Y();
...
>   	// No casr required for conversion from pointer to void*
>   	void* p = xp;
...
>   ```
>
>   **reinterpret_cast() 的一种用法是与普通可复制类型的二进制 I/O 一起使用。**
>
>   所谓普通可复制类型，是指构成对象的基础字节的类型可复制到数组中。如果此后要将数组的数据复制回对象，对象将保持其原始值。例如，可将这种类型的单独字节写入文件中。将文件读入内存时，可使用 reinterpret_cast() 来正确地解释从文件读入的字节。
>
>   **但一般而言，使用 reinterpret_cast() 时要特别小心，因为在执行转换时不会执行任何类型检测。**
>
>   **警告：**
>
>   理论上，还可以使用 reinterpret_cast() 将指针转换为 int，或将 int 转换为指针，只能将指针转化为足以容纳它的 int 类型。例如，使用 reinterpret_cast() 将 64 位的指针转化为 32 位的整数会导致编译错误。

##### 4.dynamic_cast()

>   dynamic_cast() 为继承层次解构内的类型转换提供了运行时检测，可用它转换指针或引用。dynamic_cast() 在运行时检测底层对象的类型信息。
>
>   如果类型转换没有意义，dynamic_cast() 将返回一个空指针(用于指针)或抛出一个 std::bad_cast 异常(用于引用) 。
>
>   例如，假设具有以下类层次结构：
>
>   ```C++
...
>   ```
>
>   如第 10 章所述，运行时类型信息存储在对象的虚表中。因此，为使用 dynamic_cast()，类至少要有一个虚方法。如果类不具有虚表，尝试使用 dynamic_cast() 将导致编译错误。例如，在 Microsoft VC++ 中，将给出以下错误：
>
>   ```c++
>   error C2683: 'dynamic_cast' : 'MyClass' is not a ploymorphic type.
>   ```
...
>   下表总结了不同情形下应该使用的类型转换：
>
>   ![image-20241014214009388](https://s2.loli.net/2024/10/14/JHmt9GrxUy5XzsE.png)

****

### 11.4作用域解析

>   **C++ 程序员必须熟悉作用域(scope)的概念。程序中的所有名称，包括变量、函数和类名，都具有某种作用域。**
>
>   **可使用名称空间、函数定义、花括号界定的块和类定义创建作用域。**
>
>   在一个 for 循环的初始化语句中，初始化的变量的作用域仅限于这个 for 循环，在这个 for 循环之外不可见。与此类似，C++17 为 if 和 switch 语句引入了初始化器(见第 1 章)。**在这种初始化器中，初始化的变量的作用域限于 if 和 switch 语句，在语句之外不可见。**
>
>   在 C++ 中，当编译器遇到一个标识符时，它需要决定应该调用哪个函数。这种选择过程叫做**名称解析**。**当试图访问某个变量、函数或类时，首先在最近的作用域内查找这个名称，然后查找相邻的作用域，以此类推，直到全局作用域。**任何不在名称空间、函数、花括号界定的块和类中的名称都被认为在全局作用域内。如果在全局作用域内也找不到这个名称，编译器会给出**未定义符号错误**，即：
>
>   C++ 会按照以下顺序进行名称解析：
>
>   1.  **当前作用域**：首先查找当前作用域内的定义。当前作用域可能是局部变量、局部函数或局部类等。
>   2.  **外围作用域**：如果当前作用域没有找到匹配的标识符，编译器会向外层作用域查找。外层可能是包含这个函数的类或命名空间。
>   3.  **全局作用域**：如果在局部和外围作用域中找不到匹配的标识符，编译器会查找全局作用域中的定义。
>   4.  **外部命名空间**：如果使用了 `using namespace` 引入了外部命名空间，编译器会将这个命名空间中的标识符引入当前作用域。
>   5.  **匿名命名空间**：匿名命名空间中的标识符只对所在的翻译单元可见，编译器会优先解析局部匿名命名空间中的标识符。
>
>   **有时某个作用域内的名称会隐藏其他作用域内的同一名称。**
>
>   **在另一些情况下，程序的特定行中的默认作用域解析并不包含需要的作用域。**
>
>   **如果不想用默认的作用域解析某个名称，就可以使用作用域解析运算符 :: 和特定的作用域限定这个名称。**
>
>   例如，为访问类的静态方法，第一种方法是：将类名(方法的作用域)和作用域解析运算符放在方法名的前面，第二种方法是：通过类的对象访问这个静态方法。下例演示了这两种方法，这个示例定义了一个具有静态方法 get() 的Demo类、一个具有全局作用域的 get() 函数以及一个位于 NS 名称空间的 get() 函数：
>
>   `````C++
>   class Demo
>   {
...
>   `````
>
>   **全局作用域没有名称，但可使用作用域解析运算符本身(没有名称前缀来访问)。**
>
>   可采用以下方法调用不同的 get() 函数，在这个示例中，代码本身在 main() 函数中，main 函数总是位于全局作用域内：
>
>   ```c++
>   int main()
>   {
...
>       Demo d;
>       std::cout << pd->get() << std::endl;	// print 5
>       std::cout << d.get() << std::endl;	// print 5
>       std::cout << NS::get() << std::endl;	// print 20
>       std::cout << Demo::get() << std::endl;	// print 5
>       std::cout << ::get() << std::endl;	// print 10
>       std::cout << get() << std::endl;	// print 10
>   }
>   ```
>
>   **注意：**
>
>   如果 NS 名称空间是一个匿名名称空间，下面的行将导致名称解析歧义错误，因为在全局作用域内定义了一个 get() 函数，在匿名名称空间中也定义了一个 get() 函数。
>
>   ```c++
>   int get() 
>   {
...
>   ```
>
>   如果在 main() 函数之前使用 using 子句，这会将 `NS` 命名空间中的所有标识符引入当前作用域。也会发生同样的错误：
>
>   ```c++
>   using namespace NS;
>   ```
...
>   特性(attribute)是在源代码中添加可选信息(或者供应商指定的信息)的一种机制。在 C++11 之前，供应商决定如何指定这些信息，例如 `__attribute__` 和`__ declspec` 等。自 C++11 以后，使用两个方括号语法 [[attribute]] 支持特性。
>
>   C++ 标准只定义了 6 个标准特性，下面将详细介绍。

****

#### 5.1[[noreturn]] 特征

>   **通常，程序执行是线性的：当一个函数被调用时，它执行完毕后会返回到调用它的地方继续执行剩下的代码。而[[noreturn]] 意味着函数永远不会将控制交还调用点，永远不会交还控制权。**
>
>   **典型情况是函数导致某种终止(进程终止或线程终止)或者抛出异常。**
>
>   **`[[noreturn]]` 的主要作用就是：告诉编译器该函数不会返回控制权，因此不需要在函数终止路径上寻找返回值。这让编译器能够更好地理解函数的意图，避免发出诸如“函数未返回值”的警告，并且优化控制流分析。**
>
>   `[[noreturn]]` 能帮助编译器更好地理解程序的意图，减少无用的警告，并优化编译器的控制流分析。下面是一个例子：
>
>   ```c++
...
>          forceProgramTermination();	// This terminates, but compiler doesn't know without [[noreturn]]
>      } 
>      else 
>      {
>          bool isLicensed = false;
>          // If dongle available, perform license check of the given feature...
>          return isLicensed;	// Some other return for example.
>      }
>   }
>   
>   int main() 
...
>   ```
>
>   这个代码片段可正常编译，不会发出任何警告或错误。但如果删除 [[noretum]] 特性，编译器将生成以下警告消息(Visual C++的输出)：
>
>   ```c++
...
>   **应用场景：**
>
>   -   **程序终止**：像 `std::exit()` 或 `abort()` 这样的函数会导致程序终止，因此它们永远不会返回给调用它们的代码。在这种情况下，标记 `[[noreturn]]` 告诉编译器不要期望该函数返回，这样可以避免一些编译器警告，例如警告某个函数未返回值。如代码中的 `forceProgramTermination()`，调用 `std::exit()` 后程序立即退出，根本不需要返回到调用它的地方。标记 `[[noreturn]]` 后，编译器理解这是有意为之，不会提示缺少返回值的警告。
>
>   -   **抛出异常**：如果一个函数只负责抛出异常，也不会返回到调用它的代码，则可以使用 `[[noreturn]]` 标记。编译器因此知道控制流不会回到函数调用处。在异常处理机制中，如果有个函数专门负责抛出异常，如 `throw std::runtime_error("Error");`，可以用 `[[noreturn]]` 表明该函数不会返回到调用者。
...
>   **从 C++17 开始，可使用 [[fallthrough]] 特性告诉编译器：在 switch 语句中，fall through 是有意安排的。**如果没有指定该特性，用以说明这是有意为之的，编译器将给出警告消息。
>
>   不需要为空的 case 分支指定这个特性，例如:  
>
>   ```c++
>   #include <iostream>
>   
...
>      case 1:
>          std::cout << "One\n";
>          [[fallthrough]];  // Intentionally fall through to the next case
>      case 2:
>          std::cout << "Two\n";
>          break;  // Stop the fallthrough here
>      case 3:
>          std::cout << "Three\n";
>          [[fallthrough]];  // Intentionally fall through to the next case
>      case 4:
...
>   ```
>
>   **`[[fallthrough]]` 仅仅表示当前 `case` 应该贯穿到紧接的下一个 `case`，它只影响下一个 case。如果需要让多个 `case` 连续贯穿，必须在每个需要贯穿的 `case` 之后使用 `[[fallthrough]]`，或者不使用 `break`。****
...
>   ```
>
>   如果将编译器警告级别设置得足够高，这个函数定义将导致两个编译器警告。例如，Microsoft VC++ 给出下列警告：
>
>   ```c++
...
>   ```
>
>   这里给第二个参数标记了 [[maybe_unused]] 特性。编译器只为 param1 显示警告消息：
>
>   ```c++
>   warning C4100: 'param1': unreferenced formal parameter
>   ```
...
#### 5.6[[carries_dependency]] 特征

>   [[carries_dependency]]  是一个相当怪异、冷门的特性，通常在涉及**生产者-消费者模式**或**线程之间的低级同步操作**时才可能用到。

###### 依赖

>   为解释清楚这一特性，需要进一步了解依赖的涵义：**在多线程编程中，依赖通常指的是某个线程的行为或计算结果依赖于另一个线程的行为或计算结果。依赖关系会影响线程间的数据共享、内存访问以及同步操作。**
>
>   ****
>
>   **依赖(dependency)**的核心概念是：**线程 A 的执行结果或数据，必须在被线程 B 使用之前先完成。这种先后顺序的约束是为了确保程序的正确性。**通常在涉及**生产者-消费者模式**或**线程之间的低级同步操作**时才可能用到。例如：
>
>   - 如果一个线程负责生成数据，而另一个线程需要消费这些数据，则消费者线程在消费之前依赖于生产者线程完成数据生成。
>   - 如果一个线程修改某个共享变量，而另一个线程读取这个变量，读取的线程在读取之前依赖于写入线程。
>
>   在多线程环境中，依赖主要表现为对**共享数据**的操作顺序、线程间的**数据一致性**、以及**内存模型**的操作约束。
>
>   ****
>
>   **在底层系统中，依赖主要通过以下几个方面来体现：**
>
>   **1.内存模型和顺序保证**
>
>   现代处理器和编译器可能会对程序中的内存操作进行重排序(instruction reordering)，以提高效率。这意味着即便在代码中显式地定义了操作顺序，实际执行顺序可能会不同。如果存在依赖关系，而没有恰当的同步机制，那么重排序可能导致线程之间看到不一致的数据。
>
>   例如，处理器可能会将对某个内存地址的读取操作提前执行，从而导致线程读取到一个还未更新的数据。这种情况下，依赖会受到破坏。
>
>    **2.内存屏障(Memory Fences)**
>
>   为了防止重排序和保证依赖关系，系统提供了一种低级别的同步机制——**内存屏障**(memory fences)。内存屏障是一个 CPU 指令，用于确保在屏障前的所有内存操作在执行屏障之后的操作之前完成。它通过硬件和编译器强制维护操作顺序，从而保证数据的一致性。
>
>   内存屏障是依赖关系的底层表现之一，它告诉处理器和编译器某些操作不能重排序，以保证不同线程对共享数据的正确访问顺序。
>
>   **3.缓存一致性协议**
>
>   处理器之间共享的内存通常会被缓存(cache)在每个 CPU 核心的本地缓存中。多核系统中，依赖关系体现在缓存一致性协议(如 MESI 协议)上。缓存一致性协议确保每个线程都能够看到某个共享变量的最新值，从而维护跨线程的依赖。
>
>    **3.锁和条件变量**
>
>   高级别的同步原语如**锁(lock)**和**条件变量(condition variables)**，在代码中用于显式地表达线程之间的依赖。锁确保了对共享资源的独占访问，而条件变量则用于在线程之间传递依赖信号。例如：
>
>   - 一个生产者线程生成数据并通知消费者线程，可以通过条件变量来实现依赖传播。
>   - 锁则可以防止多个线程同时修改共享数据，从而避免依赖被破坏。
>
>   ****
>
>   **依赖关系在代码中通常通过以下方式体现和管理：**
>
>   **1.锁(mutexes)**
>
>   锁用于保护临界区，确保共享数据在同一时间只能被一个线程修改。锁是维护线程依赖的基础工具。以下是一个使用锁来管理依赖的简单例子：
>
>   ```cpp
>   std::mutex mtx;
>   int shared_data = 0;
...
>       shared_data = 42;  // 生产者线程更新数据
>   }
>   
>   void consumer() {
>       std::lock_guard<std::mutex> lock(mtx);
>       std::cout << shared_data << std::endl;  // 消费者线程读取数据
>   }
>   ```
>
>   在这个例子中，`consumer` 线程依赖于 `producer` 线程完成对 `shared_data` 的修改。通过锁保证了线程间的依赖关系不会因并发访问而被破坏。
>
>   **2.条件变量**
>
>   条件变量用于在两个线程之间传递依赖信号，一个线程等待某个条件成立后继续执行。以下是使用条件变量的示例：
>
>   ```cpp
>   std::mutex mtx;
...
>   void consumer() {
>       std::unique_lock<std::mutex> lock(mtx);
>       cv.wait(lock, [] { return ready; });  // 等待生产者信号
>       std::cout << data << std::endl;
>   }
>   ```
>
>   这里，`consumer` 线程在 `producer` 线程完成数据生成之前不能继续，因此 `consumer` 依赖于 `producer` 的完成状态。条件变量帮助在线程间传递这种依赖信号。
>
>    **3.原子操作**
>
>   原子操作提供了一种无需锁的线程同步方式，它在硬件层面保证操作的不可分割性，避免数据竞争。例如，`std::atomic` 可以用于维护简单的依赖关系：
>
>   ```cpp
>   std::atomic<int> shared_data(0);
>   
>   void producer() {
>       shared_data.store(42, std::memory_order_release);  // 生产者更新数据
>   }
>   
>   void consumer() {
>       int value = shared_data.load(std::memory_order_acquire);  // 消费者读取数据
...
>    **依赖关系的总结：**
>
>   - **依赖**：表示一个线程的行为(如读取、写入某个变量)依赖于另一个线程的行为先完成。
>   - **底层体现**：依赖关系在底层通过内存屏障、缓存一致性协议、编译器的重排序规则以及处理器指令体现。
>   - **代码变化**：依赖通过锁、条件变量、原子操作和内存模型中的同步指令来表达和控制。在跨线程的依赖传递中，维护操作顺序是关键。
>
>   通过合理的依赖管理，可以确保多线程程序的**正确性**和**效率**，并且减少不必要的同步开销。

>   这一特性提供的优化提示对于**弱内存模型**架构(如 ARM 或 PowerPC)更有意义，在这种架构中，依赖关系的管理是硬件优化的重要因素。主要作用是确保在线程之间传递数据时，编译器和处理器不会做一些 **过度聪明** 的优化，破坏线程之间的依赖关系，用于以下场景：
>
>   -   **线程同步**： 在多线程编程中，生产者线程可能生成某个数据，并且消费者线程依赖于该数据。使用 `[[carries_dependency]]` 可以确保传递过程中，这种依赖关系被保留，不被编译器优化破坏。
>   -   **锁与解锁机制的优化**： 在某些情况下，当线程锁定或解锁时，它可能依赖于之前或之后的某些操作。`[[carries_dependency]]` 可以告知编译器这些操作携带依赖关系，以防重排优化。

****

#### 5.7供应商专用特性

>   大多数特性是供应商指定的拓展。建议供应商不要使用给出的特性来改变程序的涵义，而是使用它们帮助编译器优化代码或者检测代码错误。例如，编译器可以通过特性告诉程序如何优化运行速度，但不应让某个特性改变程序逻辑的执行方式。这是为了保持程序在不同编译器上的行为一致性。由于不同编译器可能会有不同的扩展，如果不加限定或标准化，可能不同供应商的特性会导致冲突。这部分是针对编译器开发者的建议，而不是对普通程序员的要求。普通程序员在编写代码时一般不需要直接与编译器的开发者打交道。但是，程序员可以通过合理使用标准的 C++ 特性(而非依赖供应商的扩展)来确保代码的可移植性。例如：
>
>   ```c++
>   [[clang::noduplicate]]
>   ```
...
>   C++ 有许多可在代码中使用的标准字面量(literal)，如下所示：
>
>   -   'a' ：字符
>   -   "character array" ：以 \0 结尾的字符数组(C 风格的字符串)
>   -   3.14f ：浮点数
>   -   0xabc ：十六进制值
>
>   **C++11 允许定义自己的字面量，即：用户自定义字面量(User-defined Literals， UDL)。**
>
>   **用户定义的字面量应该以下划线开头，下划线之后的第一个字符必须小写，例如：`_i`、`_s`、`_km`、`_mile` 等，可通过编写字面量运算符(literal operators) 来实现。**
>
>   **字面量运算符能以 生(raw)模式 或 熟(cooked)模式 运行：**
>
>   -   在 **生模式** 中，字面量运算符接收一个字符序列；
>   -   在 **熟模式** 中，字面量运算符接收一种经过解释的特定类型。
>
>   例如，考虑 C++ 字面量 123：生模式字面量运算符会将其作为字符 '1'、'2'、'3'；而熟模式字面量会将其作为整数 123。另一个示例，考虑 C++ 字面量 0x23：生模式字面量运算符将接收字符 '0'、'x'、'2'、'3'；而熟模式字面量运算符将接收整数 35。最后一个示例，考虑 C++ 字面量 3.14，生模式字面量运算符将接收字符 '3'、'.'、'1'、'4'；而熟模式字面量运算符将接收浮点值 3.14。
>
>   熟模式字面量运算符应该具有：
>
>   -   用来处理数值，需要一个 unsigned long long、long double、char、wchar_t、charl6_t 或 char32_t 类型的参数。
>       -   `unsigned long long`: 对应整数字面量（例如 `42_ull`，其中 `_ull` 是自定义后缀）。
>       -   `long double`: 对应浮点数字面量（例如 `42.0_ld`）。
>       -   `char`、`wchar_t`、`char16_t`、`char32_t`: 对应字符字面量，用于处理单个字符。
>   -   对于字符串字面量的处理，需要两个参数：第一个参数是字符数组，第二个参数是字符数组的长度(不包括空终止符 `\0`)，用来处理字符串，例如：const char* str 和 size_t len。
>
>   下面使用熟模式字面量运算符实现了用户自定义的字面量 `_i`，`_i` 用来定义一个复数字面量。
>
>   ```c++
>   std::complex<long double> opertor"" _i(long double d)
>   {
...
>   ```
>
>   另一个示例用熟模式字面量运算符实现了用户定义的字面量 `_s`，用于定义 std::string 字面量：
>
>   ```c++
...
>   ```
>
>   如果没有 `_s` 字面量，自动推导的类型将会是 const char*。
>
>   ```c++
>   auto str3 = "Hello World";	// str3 has as type const char*
>   ```
>
>   **生模式字面量运算符需要一个 const char* 类型的参数，这是一个以 `\0` 结尾的 C 风格字符串。**下面的示例定义了字面量 `_i`，此时使用的是生模式字面量运算符：
>
>   ```c++
...
##### ※这上面的部分存疑，部分概念有些模糊，对于我而言，之后需要修改

#### 标准的用户定义字面量

>   C++ 定义了如下标准的用户定义字面量。注意，这些标准的用户定义字面量并非以下面线开头：
>
>   - **`s`** 用于创建 `std::string`
>       - 例：`auto myString = "Hello World"s;`
>       - 需要 `using namespace std::string_literals;`
>
>   - **`sv`** 用于创建 `std::string_view`
>       - 例：`auto myStringView = "Hello World"sv;`
>       - 需要 `using namespace std::string_view_literals;`
>
>   - **`h`**、**`min`**、**`s`**、**`ms`**、**`us`**、**`ns`** 用于创建 `std::chrono::duration` 时间段，参见第 20 章
>       - 例：`auto myDuration = 42min;`
>       - 需要 `using namespace std::chrono_literals;`
>
>   - **`i`**、**`il`**、**`if`** 分别用于创建复数 `complex<double>`、`complex<long double>` 和 `complex<float>`
>       - 例：`auto myComplexNumber = 1.3i;`
>       - 需要 `using namespace std::complex_literals;`
>
>   使用 `using namespace std;` 也使得这些字面量变得可用。
...
#### 6.1std:promise 和 std:future  

>   

****

#### 6.2std:packaged_task
